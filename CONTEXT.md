...
Current instances: `TaskStore` (tasks collection, parameterized `fetch`/`stream`), `ProjectStore` (projects collection, parameterless `fetch`/`stream` — caller filters locally). `teams` does not have one yet — known debt, not an oversight.
